...
<?php
  if ($_POST["comment"] != null) {
    $comsave = @file("comment/comment08.txt");
    $comsw = fopen("comment/comment08.txt","a");
    fputs($comsw,$_POST["comment"]."\r\n");
    fclose($comsw);

    $usersave = @file("comment/user08.txt");
    $usersw = fopen("comment/user08.txt","a");
    fputs($usersw,$_SESSION["name"]."\r\n");
    fclose($usersw);

    $timesave = @file("comment/time08.txt");
    $timesw = fopen("comment/time08.txt","a");
    fputs($timesw,date('Y-m-j')."\r\n");
    fclose($timesw);

    echo "<script type='text/javascript'>";
		echo "function refresh() {window.location.href = \"c08.php\";}";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <img src="images/course/c08.jpg" alt="post-thumb" class="img-fluid w-100 mb-3">
        <h3 class="font-tertiary mb-5">飲食訓練課程</h3>
        <div class="content">
          <p>在減脂這條路上，你是不是常常也有這些疑惑：少吃澱粉就等於能減脂？吃了這麼少，運動都累趴了，體重還卻沒變化？
            低醣飲食？生酮？168？到底哪種飲食才能減脂快又好？這堂課能利用最健康且正確的知識來帶你瘦身，誰說減脂一定要挨餓！<br><br>
            在這個三週的減脂營中，我將給你：<br>
            減脂菜單：每週一次，當週外食或自煮都任由你決定。<br>
            訓練菜單：做二休一，在家也可以輕鬆做輕鬆增肌減脂。<br>
            減脂手冊：記錄每天的身體變化輕鬆邁向更好的自己。
          </p>
        </div>
      </div>
    </div>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <div class="p-5 mb-4">
          <?php
            $userload = @file("comment/user08.txt");
            $comload = @file("comment/comment08.txt");
            $timeload = @file("comment/time08.txt");
        ...
        <form action="c08.php" method="post" class="row">
          <div class="col-12">
            <?php
              if ($_SESSION['check_status'] == "login_ok") {
