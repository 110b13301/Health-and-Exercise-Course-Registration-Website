...
<?php
  if ($_POST["comment"] != null) {
    $comsave = @file("comment/comment02.txt");
    $comsw = fopen("comment/comment02.txt","a");
    fputs($comsw,$_POST["comment"]."\r\n");
    fclose($comsw);

    $usersave = @file("comment/user02.txt");
    $usersw = fopen("comment/user02.txt","a");
    fputs($usersw,$_SESSION["name"]."\r\n");
    fclose($usersw);

    $timesave = @file("comment/time02.txt");
    $timesw = fopen("comment/time02.txt","a");
    fputs($timesw,date('Y-m-j')."\r\n");
    fclose($timesw);

    echo "<script type='text/javascript'>";
		echo "function refresh() {window.location.href = \"c02.php\";}";
...
<section class="section">
  <div class="container">
    <div class="row">
      ...
      <img src="images/course/c02.jpg" alt="post-thumb" class="img-fluid w-100 mb-3">
        <h3 class="font-tertiary mb-5">增肌減脂課程</h3>
        <div class="content">
          <p>想培養運動但不知道怎麼開始？明明有運動，體重和體態卻聞風不動？減脂卡關下不去，害怕功虧一簣好心急？
            我們邀請了5位專業運動營養教練！陪你一起努力，比昨天更進一步！<br><br>
            這堂課適合什麼人？<br>
            運動初學者：平常不運動，但想開始培養運動習慣。<br>
            認真爬文研究，實行起來效果卻不理想：明明有運動，也有飲食控制，但就是瘦不下去或是壯不起來。<br>
            健身停滯、卡關：減脂卡關，需要新的刺激或是想嘗試不同的運動變化。<br>
          </p>
        </div>
      </div>
    </div>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <div class="p-5 mb-4">
          <?php
            $userload = @file("comment/user02.txt");
            $comload = @file("comment/comment02.txt");
            $timeload = @file("comment/time02.txt");
        ...
        <form action="c02.php" method="post" class="row">
          <div class="col-12">
            <?php
              if ($_SESSION['check_status'] == "login_ok") {
