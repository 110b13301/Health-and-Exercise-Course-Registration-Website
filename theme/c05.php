...
<?php
  if ($_POST["comment"] != null) {
    $comsave = @file("comment/comment05.txt");
    $comsw = fopen("comment/comment05.txt","a");
    fputs($comsw,$_POST["comment"]."\r\n");
    fclose($comsw);

    $usersave = @file("comment/user05.txt");
    $usersw = fopen("comment/user05.txt","a");
    fputs($usersw,$_SESSION["name"]."\r\n");
    fclose($usersw);

    $timesave = @file("comment/time05.txt");
    $timesw = fopen("comment/time05.txt","a");
    fputs($timesw,date('Y-m-j')."\r\n");
    fclose($timesw);

    echo "<script type='text/javascript'>";
		echo "function refresh() {window.location.href = \"c05.php\";}";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <img src="images/course/c05.jpg" alt="post-thumb" class="img-fluid w-100 mb-3">
        <h3 class="font-tertiary mb-5">芭蕾提斯課程</h3>
        <div class="content">
          <p>這是一堂人人都適用的優雅燃脂課，由舞蹈教學經驗豐富的專家研發，藉由古典芭蕾特色的墊上訓練，針對身體核心與三大部位重點菜單，
            不需要舞蹈基礎，零器材為你打造自信體態！讓你輕鬆在家按照個人步調反覆練習，徹底學會日常就能做的低衝擊，高強度運動！
          </p>
        </div>
      </div>
    </div>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <div class="p-5 mb-4">
          <?php
            $userload = @file("comment/user05.txt");
            $comload = @file("comment/comment05.txt");
            $timeload = @file("comment/time05.txt");
        ...
        <form action="c05.php" method="post" class="row">
          <div class="col-12">
            <?php
              if ($_SESSION['check_status'] == "login_ok") {
