...
<?php
  if ($_POST["comment"] != null) {
    $comsave = @file("comment/comment04.txt");
    $comsw = fopen("comment/comment04.txt","a");
    fputs($comsw,$_POST["comment"]."\r\n");
    fclose($comsw);

    $usersave = @file("comment/user04.txt");
    $usersw = fopen("comment/user04.txt","a");
    fputs($usersw,$_SESSION["name"]."\r\n");
    fclose($usersw);

    $timesave = @file("comment/time04.txt");
    $timesw = fopen("comment/time04.txt","a");
    fputs($timesw,date('Y-m-j')."\r\n");
    fclose($timesw);

    echo "<script type='text/javascript'>";
		echo "function refresh() {window.location.href = \"c04.php\";}";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <img src="images/course/c04.jpg" alt="post-thumb" class="img-fluid w-100 mb-3">
        <h3 class="font-tertiary mb-5">精實線條課程</h3>
        <div class="content">
          <p>減重後怎麼樣才可以不復胖？為什麼練了半天，卻一直練不出想要的線條？其實這些問題，都是不懂得怎麼正確的增肌所造成的。
            這門課程會交給你正確的增肌觀念，以及日常中能夠落實的飲食原則，同時搭配特別設計的運動課表，只要10週，你就能看見自己的改變！<br><br>
            課程分成四個單元：<br>
            1.重新理解增肌的觀念<br>
            2.有效精實線條的增肌運動<br>
            3.增肌飲食與減脂飲食<br>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <div class="p-5 mb-4">
          <?php
            $userload = @file("comment/user04.txt");
            $comload = @file("comment/comment04.txt");
            $timeload = @file("comment/time04.txt");
        ...
        <form action="c04.php" method="post" class="row">
          <div class="col-12">
            <?php
              if ($_SESSION['check_status'] == "login_ok") {
