...
<?php
  if ($_POST["comment"] != null) {
    $comsave = @file("comment/comment06.txt");
    $comsw = fopen("comment/comment06.txt","a");
    fputs($comsw,$_POST["comment"]."\r\n");
    fclose($comsw);

    $usersave = @file("comment/user06.txt");
    $usersw = fopen("comment/user06.txt","a");
    fputs($usersw,$_SESSION["name"]."\r\n");
    fclose($usersw);

    $timesave = @file("comment/time06.txt");
    $timesw = fopen("comment/time06.txt","a");
    fputs($timesw,date('Y-m-j')."\r\n");
    fclose($timesw);

    echo "<script type='text/javascript'>";
		echo "function refresh() {window.location.href = \"c06.php\";}";
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <img src="images/course/c06.jpg" alt="post-thumb" class="img-fluid w-100 mb-3">
        <h3 class="font-tertiary mb-5">瑜珈課程</h3>
        <div class="content">
          <p>把瑜珈練習打包進你的日常生活，探索瑜珈的各種面相。<br><br>
            阿育吠陀是來自印度的養生哲學，運用瑜珈、呼吸與冥想，達到身心靈的平衡和諧。本瑜珈課遵循阿育吠陀養生概念、依四季能量變化而設計，
            為你調配最適合你的養生瑜珈練習。在這個瑜珈教室裡，包含了三堂不同長度的完整瑜珈練習以及該主題練習的相關解說影片。<br><br>
            這是一門不趕、不擠、隨開隨上的瑜珈課，不用再煩惱瑜珈課程表永遠對不上自己的行程表、也不用再躊躇於大教室裡的人擠人，打破傳統的瑜珈練習概念，
            讓你走到哪練習到哪。或許你不是每天都有足夠的時間、精力或興趣進行完整的一堂瑜珈課，透過不同長度與主題的瑜珈課以及冥想導引，
            讓你主導安排最適合自己的練習。<br><br>
            瑜珈體位幫你解開身體打結的地方，呼吸法幫你安神靜心，冥想帶領你內觀自在，哲學則是透過前人的智慧啟發你內心的明燈，照亮你人生的道路。
          </p>
        </div>
      </div>
    </div>
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-10 mx-auto">
        <div class="p-5 mb-4">
          <?php
            $userload = @file("comment/user06.txt");
            $comload = @file("comment/comment06.txt");
            $timeload = @file("comment/time06.txt");
        ...
        <form action="c06.php" method="post" class="row">
          <div class="col-12">
            <?php
              if ($_SESSION['check_status'] == "login_ok") {
